...
import { GET_ABOUT_FOOTER } from "../../graphql/Queries";
import { CircularProgress } from "@mui/material";

import { Partner, Project } from "../Links/About/AboutComponents";
const AboutFooter = () => {
  const { loading, error, data } = useQuery(GET_ABOUT_FOOTER);

  if (loading) return <CircularProgress disableShrink className="m-8" />;
  if (error) return `Error! ${error.message}`;

  const acf = data?.layouts?.nodes[0]?.miseEnPage?.apropos?.footer;
